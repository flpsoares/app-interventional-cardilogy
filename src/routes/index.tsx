      ...
      .then(async (token) => {
        if (token) {
          ...
          if (userId) {
            ...
            try {
              await setDoc(userDoc, { notificationToken: token }, { merge: true });
            } catch (error) {
              console.error(error);
            }
          }
  ...
  useEffect(() => {
    if (user !== undefined) {
      ...
      if (auth.currentUser) {
        ...
        const unsubscribe = onSnapshot(userDoc, (querySnapshot) => {
          const expirationDate = querySnapshot.data()?.expiration_date;
          const dateNow = moment().tz('America/Sao_Paulo');

          if (expirationDate) {
            const isExpired = moment(expirationDate).diff(dateNow, 'days') < 0;

            if (isExpired) {
              setDoc(userDoc, { isSubscriber: false }, { merge: true });
            }
          }
        });
