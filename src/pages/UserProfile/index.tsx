...
import { collection, doc, query, where, orderBy, onSnapshot, getDoc } from 'firebase/firestore';
import {
  Banner,
  Container,
  Email,
...
export const UserProfile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // user
    const userDocRef = doc(database, 'users', route.params.id);
    ...
    // followers
    const followersQuery = query(collection(database, `users/${route.params.id}/followers`));

    const unsubscribeFollowers = onSnapshot(followersQuery, (querySnapshot) => {
    ...
    // followings
    const followingsQuery = query(collection(database, `users/${route.params.id}/following`));

    const unsubscribeFollowings = onSnapshot(followingsQuery, (querySnapshot) => {
      if (querySnapshot.docs[0] === undefined) {
